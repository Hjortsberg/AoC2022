...
var inputString = @"jNNBMTNzvTqhQLhQLMQL
...
var parsedRucksacks = Parser.ParseRucksack(inputString);
var parsedGroups = Parser.ParseRucksackForGroup(inputString);
var prioritySum = new ConcurrentBag<int>();
var prioritySumProblemTwo = new ConcurrentBag<int>();

Parallel.ForEach(parsedRucksacks, rucksack =>
{
    foreach (var character in rucksack.firstCompartment)
    {
        if (rucksack.secondCompartment.Contains(character))
            prioritySum.Add(Char.IsLower(character) ? character - 96 : character - 38);
    }
});

Parallel.ForEach(parsedGroups, group =>
{
    foreach (var character in group.Elf1)
    {
        if (group.Elf2.Contains(character) && group.Elf3.Contains(character))
           prioritySumProblemTwo.Add(Char.IsLower(character) ? character - 96 : character - 38);
    }
});

Console.WriteLine(prioritySum.Sum());
Console.WriteLine(prioritySumProblemTwo.Sum());